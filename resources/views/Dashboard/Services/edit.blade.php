...
<div class="modal fade" id="edit{{ $service->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('Services.update' , [$service->id]) }}" method="POST" class="form bg-white p-6 border-1" >
                {{ method_field('patch') }}
                {{ csrf_field() }}
                @csrf
                <div class="modal-body">
                    ...
                    <label for="name">{{trans('Services.name2')}}</label>
                    <input type="text" name="name_en" id="name" value="{{$service->name_en}}" class="form-control"><br>
